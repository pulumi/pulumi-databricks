...
//
// > **Upgrading from v1.114.0**: state written by v1.114.0 encodes `providerConfig` as a single object instead of a list. After upgrading the provider, edit each `QualityMonitor` instance in your state file to convert `"providerConfig": {"workspaceId": "X"}` to `"providerConfig": null` (recommended if you didn't set `providerConfig` in HCL) or to `"providerConfig": [{"workspaceId": "X"}]` (if you did). Without this edit, `pulumi preview` fails with `Error decoding ... missing expected [`. Users on v1.113.0 are unaffected.
//
// ## Example Usage
//
// ```go
