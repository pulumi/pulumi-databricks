// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.databricks.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class PostgresDatabaseSpec {
    private @Nullable String postgresDatabase;
    private String role;

    private PostgresDatabaseSpec() {}
    public Optional<String> postgresDatabase() {
        return Optional.ofNullable(this.postgresDatabase);
    }
    public String role() {
        return this.role;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(PostgresDatabaseSpec defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String postgresDatabase;
        private String role;
        public Builder() {}
        public Builder(PostgresDatabaseSpec defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.postgresDatabase = defaults.postgresDatabase;
    	      this.role = defaults.role;
        }

        @CustomType.Setter
        public Builder postgresDatabase(@Nullable String postgresDatabase) {

            this.postgresDatabase = postgresDatabase;
            return this;
        }
        @CustomType.Setter
        public Builder role(String role) {
            if (role == null) {
              throw new MissingRequiredPropertyException("PostgresDatabaseSpec", "role");
            }
            this.role = role;
            return this;
        }
        public PostgresDatabaseSpec build() {
            final var _resultValue = new PostgresDatabaseSpec();
            _resultValue.postgresDatabase = postgresDatabase;
            _resultValue.role = role;
            return _resultValue;
        }
    }
}
