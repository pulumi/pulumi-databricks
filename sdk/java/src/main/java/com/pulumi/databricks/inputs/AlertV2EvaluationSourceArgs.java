// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.databricks.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class AlertV2EvaluationSourceArgs extends com.pulumi.resources.ResourceArgs {

    public static final AlertV2EvaluationSourceArgs Empty = new AlertV2EvaluationSourceArgs();

    /**
     * Possible values are: `AVG`, `COUNT`, `COUNT_DISTINCT`, `MAX`, `MEDIAN`, `MIN`, `STDDEV`, `SUM`
     * 
     */
    @Import(name="aggregation")
    private @Nullable Output<String> aggregation;

    /**
     * @return Possible values are: `AVG`, `COUNT`, `COUNT_DISTINCT`, `MAX`, `MEDIAN`, `MIN`, `STDDEV`, `SUM`
     * 
     */
    public Optional<Output<String>> aggregation() {
        return Optional.ofNullable(this.aggregation);
    }

    @Import(name="display")
    private @Nullable Output<String> display;

    public Optional<Output<String>> display() {
        return Optional.ofNullable(this.display);
    }

    @Import(name="name", required=true)
    private Output<String> name;

    public Output<String> name() {
        return this.name;
    }

    private AlertV2EvaluationSourceArgs() {}

    private AlertV2EvaluationSourceArgs(AlertV2EvaluationSourceArgs $) {
        this.aggregation = $.aggregation;
        this.display = $.display;
        this.name = $.name;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(AlertV2EvaluationSourceArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private AlertV2EvaluationSourceArgs $;

        public Builder() {
            $ = new AlertV2EvaluationSourceArgs();
        }

        public Builder(AlertV2EvaluationSourceArgs defaults) {
            $ = new AlertV2EvaluationSourceArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param aggregation Possible values are: `AVG`, `COUNT`, `COUNT_DISTINCT`, `MAX`, `MEDIAN`, `MIN`, `STDDEV`, `SUM`
         * 
         * @return builder
         * 
         */
        public Builder aggregation(@Nullable Output<String> aggregation) {
            $.aggregation = aggregation;
            return this;
        }

        /**
         * @param aggregation Possible values are: `AVG`, `COUNT`, `COUNT_DISTINCT`, `MAX`, `MEDIAN`, `MIN`, `STDDEV`, `SUM`
         * 
         * @return builder
         * 
         */
        public Builder aggregation(String aggregation) {
            return aggregation(Output.of(aggregation));
        }

        public Builder display(@Nullable Output<String> display) {
            $.display = display;
            return this;
        }

        public Builder display(String display) {
            return display(Output.of(display));
        }

        public Builder name(Output<String> name) {
            $.name = name;
            return this;
        }

        public Builder name(String name) {
            return name(Output.of(name));
        }

        public AlertV2EvaluationSourceArgs build() {
            if ($.name == null) {
                throw new MissingRequiredPropertyException("AlertV2EvaluationSourceArgs", "name");
            }
            return $;
        }
    }

}
